...
from .humanoidenv_pybullet import HumanoidEnv_PB
...
class GHumanoid_BF(mujoco_env.MujocoEnv, utils.EzPickle, ABC):
	TARGET_SHAPE = 0
	MAX_PIX_VALUE = 0

	def __init__(self):

		self.env = HumanoidEnv_PB(
		)
		...
		self.viewer = None# self.viewer = mujoco_py.MjViewer(self.env.sim)
	...
	def project_to_goal_space(self, state):
		com_or = self.get_com_or(state)

		return com_or

	def get_com_or(self, state):
		"""
...
class GHumanoidGoal_BF(GHumanoid_BF, GoalEnv, utils.EzPickle, ABC):
	def __init__(self):
		super().__init__()
	...
	def reset(self, options=None, seed: Optional[int] = None, infos=None):
		...
		return {
			...
			'achieved_goal': self.project_to_goal_space(self.env._get_obs()).copy(),
			'desired_goal': self.goal.copy(),
		}
	...
	def reset_before_reset_done(self, reset_state):
		if reset_state:
			self.reset_model()
	...
	def get_com_or(self, state):
		"""
		get center of mass orientation from full state for torso?
		"""
		com_or = state[3:7]
	...
	def get_state(self):
		state = (self.env._get_state().copy(), self.env.get_inner_state())
		return state
